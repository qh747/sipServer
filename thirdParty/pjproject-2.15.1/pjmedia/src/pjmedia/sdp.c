    ...
    PJ_TRY {
        while (!pj_scan_is_eof(&scanner)) {
                ...
                case 'b':
                    ...
                    break;
                case 'y':
                    parse_generic_line(&scanner, &session->gb28181_extend_y, &ctx);
                    break;
                case 'f':
                    parse_generic_line(&scanner, &session->gb28181_extend_f, &ctx);
                    break;
                case 'u':
                    parse_generic_line(&scanner, &session->gb28181_extend_u, &ctx);
                default:
                                                ...
                                                const pjmedia_sdp_session *rhs)
{
    ...
    /* Clone gb28181 media description */
    if (NULL != rhs->gb28181_extend_y.ptr) {
        pj_strdup(pool, &sess->gb28181_extend_y, &rhs->gb28181_extend_y);
    }

    /* Clone gb28181 ssrc */
    if (NULL != rhs->gb28181_extend_f.ptr) {
        pj_strdup(pool, &sess->gb28181_extend_f, &rhs->gb28181_extend_f);
    }

    /* Clone gb28181 uri */
    if (NULL != rhs->gb28181_extend_u.ptr) {
        pj_strdup(pool, &sess->gb28181_extend_u, &rhs->gb28181_extend_u);
    }
